...
def process_alignment(alignment_list, index_dict):
    ...
    del cigar_op_list[0]
    #deletion
    possible_sv_del = []      #posible_sv_del(>=10D) and their index in cigar
    k = 0
    ...
    for eachline in gaf_file:
        ...
        if len(batch) == batch_size:
            with ProcessPoolExecutor(max_workers=thread) as executor:
                futures = [executor.submit(process_alignment, text_list, share_index_dict) for text_list in batch]
                for future in as_completed(futures):
                    ...
                    if breakpoint_list_del:
                        for bp in breakpoint_list_del:
                            total_breakpoint_list_del.append(copy.deepcopy(bp))
                    if breakpoint_list_ins:
                        for bp in breakpoint_list_ins:
                            total_breakpoint_list_ins.append(copy.deepcopy(bp))
                    i = 0
...
def process_breakpoint(breakpoint, gfaID2bubble_dict):
    ...
    bubble_order_sorted = dict(sorted(bubble_order.items(), key=lambda item: item[1]))

    bubble_path = tuple(bubble_order_sorted.keys())
...
def breakpoint_cluster(breakpoint_list, share_gfaID2bubble_dict, thread):
    ...
    :param gfaID2bubble_dict: from "###calling bubbles", reverse dictionary of bubble_dict.
    :param thread: thread.
    ...
    if not interval_list:
        ...
        for interval in interval_list:
            if not merged_interval_list:
                ...
                if interval[0] - merged_interval_list[-1][1] > 6:
                    merged_interval_list.append(interval)
                    interval_cluster_list.append([interval])
                else:
                    merged_interval_list[-1][1] = max(interval[1], merged_interval_list[-1][1])
...
def from_interval_list_to_SV_interval_on_path_del(interval_list, graph, path, seg2base_dict, ref_seg2chr, chr2ref_seg):
    ...
    for seg in ref_path_seg:
        if graph.nodes[seg]["base"]["strand"] == "+":
    ...
    if start_on_ref_path == -1:
        ...
        with open(os.path.join(unique_tmp_subdir, cnsfa), "rt") as f_cns:
            ...
            for eachline in f_cns:
                ...
                if text[0] != ">":
                    cns += text
            alt = cns[start_on_query : end_on_query]
        start_ref_seg, end_ref_seg = -1, -1
        i = 0
        for i in range(len(path)):
            seg = path[i]
        ...
        if on_or_not_on_ref == "OnRef":
            ...
            alt = (ref_path_base[path_ins_start_index - 1], alt, "")
            ...
            if path_ins_start_index >= a:
                ...
                if len(base_) - path_ins_end_index >= b:
                    ref = ref_path_base[a - 1: len(ref_path_base) - b]
                    alt = (base_[a - 1: path_ins_start_index], alt, base_[path_ins_end_index: len(base_) - b])
                else:
                    ref = ref_path_base[a - 1: len(ref_path_base) - (len(base_) - path_ins_end_index)]
                    alt = (base_[a - 1: path_ins_start_index], alt, "")
            else:
                pos = chr2ref_seg[chrom][path_seg[0]] + path_ins_start_index
                if len(base_) - path_ins_end_index >= b:
                    ref = ref_path_base[path_ins_start_index - 1: len(ref_path_base) - b]
                    alt = ("", alt, base_[path_ins_end_index: len(base_) - b])
                else:
...
def process_cluster(bubble_path, breakpoint_list, bubble2gfaID, gfa_file, index_gfa, offset_list, ref_seg2chr,
    ...
    :param bubble2gfaID: from "calling bubbles"
    ...
    for i in range(len(bubble_path)):
        ...
        if i == 0:
            if isinstance(element, tuple) and all(isinstance(j, tuple) for j in element):
                list1, list2 = bubble2gfaID[element[0][0]][element[0][1]][1], bubble2gfaID[element[1][0]][element[1][1]][1]
                if list1[-1] == list2[0]:
                    start_seg = list2[0]
                    for seg in list2:
                        segments.add(seg)
    ...
    with open(gfa_file, "rb") as fi:
        ...
        now_segments = []
        G.nodes[start_seg]["strand"] = "+"
        segments_remained.discard(start_seg)
        seg_text_start = mm[offset_list[index_gfa[start_seg][1][0]] : offset_list[index_gfa[start_seg][1][0] + 1]].decode("utf-8")
        seg_text_start_split = seg_text_start.strip().split("\t")
        G.nodes[start_seg]["base"] = seg_text_start_split[2]
        link_index_start = index_gfa[start_seg][1][1:]
    ...
    if second_path == None:
        ...
        two_paths_intersection.update(set(second_path))
...
def detect_SV(bubble_path_clustered, cluster_result, thread,
    ...
    :param cluster_result: dict with bubble_path as key, breakpoint as value
    :return:
    '''
    vcf_info = set()
...
def main():
    ...
    with Manager() as manager:
        ...
        for bubble_path in list(cluster_result_del.keys()):
            ...
            if is_bubble_or_betweenbubble(tail_bubble) == "bubble":
                end = tail_bubble[1][1]
            else:
                end = bubble[tail_bubble[0]][tail_bubble[1]][0]
            bubble_path_del_dict[bubble_path] = (ref_tag, start, end)
        del bubble_path, head_bubble, tail_bubble, ref_tag, start, end
        ...
        now_bubble_path = []
        for i in range(len(list(bubble_path_del_dict.keys()))):
            key = list(bubble_path_del_dict.keys())[i]
            if now_bubble_path_clustered == []:
                ...
                if len(set1) < len(now_bubble_path_clustered) + len(key):
                    ...
                    now_bubble_path_clustered = list(key)
                    now_bubble_path = [key]
            if i == len(list(bubble_path_del_dict.keys())) - 1:
                bubble_path_del_clustered[tuple(now_bubble_path_clustered)] = now_bubble_path
        del bubble_path_del_dict, now_bubble_path_clustered, now_bubble_path, key, set1, element
        ...
        now_bubble_path = []
        for i in range(len(list(bubble_path_ins_dict.keys()))):
            key = list(bubble_path_ins_dict.keys())[i]
            if now_bubble_path_clustered == []:
                now_bubble_path_clustered = list(key)
                now_bubble_path = [key]
                ...
                if bubble_path_ins_dict[key][0] == now_ref_tag:
                    if bubble_path_ins_dict[key][1] >= now_start and bubble_path_ins_dict[key][2] <= now_end:
                        ...
                        now_bubble_path.append(key)
                    elif bubble_path_ins_dict[key][0] >= now_end:
                        if bubble_path_ins_dict[key][0] - now_end <= 6:
                            now_end = bubble_path_ins_dict[key][2]
                            now_bubble_path.append(key)
                    ...
                    now_ref_tag, now_start, now_end = bubble_path_ins_dict[key][0], bubble_path_ins_dict[key][1], \
                    bubble_path_ins_dict[key][2]
            if i == len(list(bubble_path_ins_dict.keys())) - 1:
                bubble_path_ins_clustered_[tuple(now_bubble_path_clustered)] = now_bubble_path
        del bubble_path_ins_dict, now_bubble_path_clustered, now_ref_tag, now_start, now_end, now_bubble_path, key, \
        ...
        for bubble_path_clustered in bubble_path_clustered_list:
            ...
            expanded_bubble_path_clustered = copy.deepcopy(list(bubble_path_clustered))
        ...
        del bubble_path_ins_clustered_, bubble_path_clustered_list, bubble_path_clustered, \
            expanded_bubble_path_clustered, head_bubble, tail_bubble, ref_tag, start, end, tuple_bubble_on_ref_tag, \
            length_tuple_bubble_on_ref_tag, index0, index8, index1, index2
        ...
        print("Detecting SV-detecting sv.")
        ...
        del index_dict_gfa
        shared_bubble2gfaID = manager.dict(bubble)
        del bubble
        ...
        with open(out_vcf_dir, "w") as fo_vcf:
            ...
            del shared_index_dict_gfa, chromsome, final_seg, so, length_final_seg
            ...
            for element in del_vcf_info:
                ...
                ID = "{}_{}_{}{}{}{}_{}_{}".format(CHROM, POS, path_strand[0], path_seg[0], path_strand[-1],
        ...
        del element, CHROM, POS, ID, REF, ALT, QUAL, FILTER, INFO_TUPLE, FORMAT, SAMPLE, NEW_SEG_LINK, \
        path_seg, path_strand, seg1, strand1, pos1, length1, seg2, strand2, pos2, length2
        ...
        del shared_chr2ref_seg, shared_ref_seg2chr, ele, eachline, text, text_split, seg_id, seg_base, cut_pos, SN, SO, \
            seg_s, strand_s, seg_e, strand_e, seg_s_, seg_e_, seg_nc, part, elem, \
            seg1, strand1, pos1, length1, seg2, strand2, pos2, length2, start_, end_, index1, strand2
